...
BaseSocket.prototype.handleMessage = function handleMessage(data, noSize) {
  var result;
  try {
    result = this._handleMessage(data, noSize);
    return cast(result);
  } catch (e) {
...
BaseSocket.prototype._handleMessage = function handleMessage(data, noSize) {
  var packet = Packet.fromRaw(data, noSize);
...
BaseSocket.prototype.dispatch = function dispatch(event, items) {
  if (items == null)
    items = [];
...
BaseSocket.prototype.listen = function listen(event, handler) {
  ...
  assert(!this.handlers[event], 'Listener already bound.');
